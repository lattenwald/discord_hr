defmodule DiscordHr.Ping do
  ...
  def interaction_react(
    ...
    with %{roles: roles, channels: %{^channel_id => %{name: channel_name}}} <-
           Cache.GuildCache.get!(guild_id) do
      {text_norole, text_alreadyhave, text_ok} =
        case locale do
          "ru" <> _ ->
            {"Нет роли для пингов в этом канале", "У вас уже есть роль `@#{channel_name}`",
             "Добавлена роль `@#{channel_name}`"}

          _ ->
            {"There's no ping role for this channel", "You already have role `@#{channel_name}`",
        ...
        {role_id, _} ->
          if Enum.member?(user_roles, role_id) do
            ...
            case Api.add_guild_member_role(
                   ...
                   user_id,
                   role_id,
                   "#{username} used /ping in #{channel_name}"
                 ) do
              {:ok} ->
  ...
  def interaction_react(
    ...
    with %{roles: roles, channels: %{^channel_id => %{name: channel_name}}} <-
           Cache.GuildCache.get!(guild_id) do
      {text_norole, text_donthave, text_ok} =
        case locale do
          "ru" <> _ ->
            {"Нет роли для пингов в этом канале", "У вас и не было роли `@#{channel_name}`",
             "Убрана роль `@#{channel_name}`"}

          _ ->
            {"There's no ping role for this channel", "You don't have role `@#{channel_name}`",
      ...
      case roles |> Enum.find(fn {_, %{name: name}} -> name == channel_name end) do
        ...
        {role_id, _} ->
          if Enum.member?(user_roles, role_id) do
            case Api.remove_guild_member_role(
                   guild_id,
                   user_id,
                   role_id,
                   "#{username} used /ping in #{channel_name}"
