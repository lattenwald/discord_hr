defmodule DiscordHr.Voice do
  ...
  def handle_component(
    ...
    with remove <- options |> Enum.filter(&Map.get(&1, :default, false)) |> Enum.map(& &1.value) do
      ...
      DiscordHr.respond_to_component(
        interaction,
        "Removed **#{length(names) - length(new_names)}** voice names"
      )
    else
      _ ->
  ...
  def handle_application_command(:get_voice_names, interaction = %{guild_id: guild_id}, _) do
    ...
    case Storage.get([guild_id, :voice_names], []) do
      ...
      names ->
        text =
          names
          |> Enum.zip(1..length(names))
          |> Enum.map(fn {name, num} -> "#{num}. `#{name}`" end)
          |> Enum.join("\n")
  ...
  def handle_application_command(:set_voice_channel, interaction = %{guild_id: guild_id}, [
        %{"id" => channel_id}
      ]) do
    with %{channels: %{^channel_id => %{name: channel_name, type: 2}}} <-
           Cache.GuildCache.get!(guild_id) do
      Storage.put([guild_id, :default_voice], channel_id)

      DiscordHr.respond_to_interaction(
        interaction,
        "Channel 🔊#{channel_name} is set as default voice channel"
      )
    else
      _ ->
        ...
        DiscordHr.respond_to_interaction(
          interaction,
          "Can't setup this channel as default voice channel"
        )
    end
  end

  def handle_application_command(:add_voice_name, interaction = %{guild_id: guild_id}, [
        %{"name" => name}
      ]) do
    names = Storage.get([guild_id, :voice_names], [])
  ...
  def handle_application_command(:delete_voice_name, interaction = %{guild_id: guild_id}, []) do
    case Storage.get([guild_id, :voice_names], []) do
      [] ->
        DiscordHr.respond_to_interaction(
          interaction,
          "There are 0 voice names, nothing to delete"
        )
  ...
  def handle_application_command(:toggle_voice_renaming, interaction = %{guild_id: guild_id}, [
        %{"on" => on}
      ]) do
    ...
    DiscordHr.respond_to_interaction(
      interaction,
      "Automatic voice renaming is turned **#{if on, do: "on", else: "off"}**"
    )
  end
  ...
  def handle_event(
        {:VOICE_STATE_UPDATE,
         %{
           ...
           member: %{user_id: userid, nick: username}
         }, _ws_state}
      ) do
    with default_voice when default_voice != nil <- Storage.get([guild_id, :default_voice]),
         {:ok, %{channels: channels, voice_states: voice_states}} <-
           Nostrum.Cache.GuildCache.get(guild_id),
         {:ok, %{parent_id: parent_id}} <- Map.fetch(channels, default_voice),
         voice_channels_of_interest <-
           channels
           |> Enum.filter(fn
             {id, %{type: 2, parent_id: ^parent_id}} -> id != default_voice
             _ -> false
           end)
         ...
         populated_voice_channels <-
           voice_states
           |> Enum.map(fn %{channel_id: channel_id} -> channel_id end)
           |> MapSet.new() do
      ...
      if channel_id == default_voice do
        channel_name =
          if Storage.get([guild_id, :voice_renaming_on], false) do
            names = Storage.get([guild_id, :voice_names], []) |> Enum.into(MapSet.new())

            existing_channel_names =
              Nostrum.Cache.GuildCache.get!(guild_id).channels
        ...
        {:ok} =
          Api.edit_channel_permissions(new_channel_id, userid, %{
            type: "member",
            allow: @new_voice_permissions
          })
  ...
  defp delete_channel_components(guild_id, selected \\ []) do
    ...
    options =
      names
      |> Enum.map(
        &%Component.Option{label: "#{&1}", value: "#{&1}", default: Enum.member?(selected, &1)}
      )

    menu =
      Component.SelectMenu.select_menu("voice:delete:select",
    ...
    [
      ...
      Component.ActionRow.action_row([
        Component.Button.interaction_button("DELETE", "voice:delete:button",
          style: 4,
          disabled: selected == []
        ),
