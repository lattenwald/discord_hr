defmodule DiscordHr.Roles do
        ...
        groups = %{} ->
          new_groups =
            ...
            |> Enum.map(fn {group_name, group} ->
              {group_name,
               Map.update(group, :roles, [], fn roles ->
                 roles |> Enum.filter(&(&1 != role_id))
               end)}
            end)
            |> Enum.into(%{})
  ...
  defp handle_application_command(:add_group, interaction, []) do
    input =
      Component.TextInput.text_input("Group name: ^[a-z][a-z0-9_]*$", "roles:add:input:name",
        min_length: 1,
    ...
    Api.create_interaction_response(interaction, %{
      type: 9,
      data: %{title: "Group name", custom_id: "roles:add:input", components: [row]}
    })
  end
  ...
  defp handle_component(
         :rename_select,
         interaction = %{guild_id: guild_id, data: %{values: selected}},
         []
       ) do
  ...
  defp handle_component(:rename_selected, interaction, []) do
    ...
    input =
      Component.TextInput.text_input("Renaming `#{name}`", "roles:rename:input",
        min_length: 1,
        max_length: @max_name_length,
        required: true
      )
    ...
    Api.create_interaction_response(interaction, %{
      ...
      data: %{
        title: "Renaming `#{name}`",
        custom_id: "roles:rename:input:#{name}",
        components: [row]
      }
    })
  end
  ...
  defp handle_component(:delete_button, interaction = %{guild_id: guild_id}, []) do
    ...
    deleted =
      names |> Enum.zip(1..1000) |> Enum.map(fn {n, i} -> "  #{i}. `#{n}`" end) |> Enum.join("\n")
      ...
      :ok ->
        ...
        buttons =
          Component.ActionRow.action_row(
            components: [
              Component.Button.interaction_button("Cancel", "roles:add:cancel_setup:#{name}",
                style: 1
              ),
        ...
        DiscordHr.respond_to_interaction(interaction, "Group `#{name}` added, setup it now?", [
          buttons
        ])
    end
  end
  ...
  defp components_selected(%Struct.Interaction{message: %{components: components}}),
    do: components_selected(components)
    ...
    acc =
      case comp do
        %{type: 3, custom_id: id, options: options} ->
          selected =
            (options || []) |> Enum.filter(&Map.get(&1, :default, false)) |> Enum.map(& &1.value)
  ...
  defp fix_values(step, values) when is_binary(step),
    do: fix_values(String.to_existing_atom(step), values)
  ...
  defp setup_group_components(guild_id),
    do: setup_group_components(guild_id, nil, Enum.at(@setup_steps, 0))

  defp setup_group_components(guild_id, name, step),
    do: setup_group_components(guild_id, name, step, nil)

  defp setup_group_components(guild_id, name, step = :name, selected) do
    case Storage.get([guild_id, @key]) |> Map.keys() do
      [name] ->
        setup_group_components(guild_id, name, next_step(step))
      ...
      names ->
        ...
        options =
          names
          |> Enum.map(
            &%Component.Option{label: &1, value: &1, default: [&1] == (selected || [name])}
          )

        menu =
          Component.SelectMenu.select_menu("roles:setup:select:#{step}:",
  ...
  defp setup_group_components(guild_id, name, step = :roles, selected) do
    ...
    all_roles =
      Cache.GuildCache.get!(guild_id).roles
      |> Enum.filter(fn {_, r} -> !r.managed and r.name != "@everyone" end)

    options =
      all_roles
      |> Enum.map(fn {id, %{name: n}} ->
        %Component.Option{label: n, value: id, default: Enum.member?(selected || roles, id)}
      end)

    menu =
      Component.SelectMenu.select_menu("roles:setup:select:#{step}:#{name}",
        placeholder: "Choose roles",
        options: options,
        min_values: 0,
        max_values: min(20, length(options))
      )
  ...
  defp setup_group_components(guild_id, name, step = :max, selected) do
    ...
    options =
      1..max(1, length(roles))
      |> Enum.map(&%Component.Option{label: "#{&1}", value: &1, default: value == &1})
  ...
  defp setup_group_components(guild_id, name, step = :done, _) do
    ...
    buttons =
      Component.ActionRow.action_row(
        components: [
          Component.Button.interaction_button("<<", "roles:setup:prev:#{step}:#{name}"),
          Component.Button.interaction_button("Done!", "roles:setup:next:#{step}:#{name}",
            disabled: true
          )
        ]
      )
  ...
  defp delete_group_components(guild_id, selected) do
    ...
    options =
      names
      |> Enum.map(&%Component.Option{label: &1, value: &1, default: Enum.member?(selected, &1)})

    menu =
      Component.SelectMenu.select_menu("roles:delete:select",
        placeholder: "Choose groups to delete",
        options: options,
        min_values: 1,
        max_values: min(10, length(names))
      )
    ...
    buttons =
      Component.ActionRow.action_row(
        components: [
          Component.Button.interaction_button("DELETE", "roles:delete:button",
            style: 4,
            disabled: selected == []
          ),
  ...
  defp rename_group_components(guild_id, selected) do
    ...
    options =
      names |> Enum.map(&%Component.Option{label: &1, value: &1, default: [&1] == selected})

    menu =
      Component.SelectMenu.select_menu("roles:rename:select",
        placeholder: "Choose group to rename",
        options: options,
        min_values: 1,
        max_values: 1
      )
    ...
    buttons =
      Component.ActionRow.action_row(
        components: [
          Component.Button.interaction_button("Rename", "roles:rename:button",
            style: 1,
            disabled: selected == []
          ),
          Component.Button.interaction_button("Cancel", "cancel", style: 1)
  ...
  defp validate_group_name(guild_id, name) do
    ...
    cond do
      ...
      not String.match?(name, @name_regex) ->
        {:error,
         "Group name `#{name}` is invalid: should consist of latin letter, numbers and underscore, starting with letter"}
