    ...
    try do
      ...
      {overrides, channel_name} =
        case channels do
          ...
          _ ->
            throw(
              {"You aren't in managed voice channel",
               "Вы не подключены к управляемому голосовому каналу"}
            )
        end

      owner_id =
        case overrides |> Enum.filter(&(&1.type == 1)) do
          [%{id: owner_id}] ->
            owner_id

          [_ | _] ->
            throw(
              {"Some problem with permission overwrites: there are too many!",
               "Слишком много оверрайдов разрешений на канал!"}
            )

          [] ->
            throw(
              {"Some problem with permission overwrites: there are none",
               "Отсутствуют оверрайды разрешений на канал"}
            )
        end
      ...
      case voice_states |> Enum.find(&(&1.channel_id == channel_id and &1.user_id == owner_id)) do
        ...
        %{} ->
          throw(
            {"Owner is still connected to channel `#{channel_name}`",
             "Хозяин канала ещё подключён к `#{channel_name}`"}
          )
      end

      Api.delete_channel_permissions(
        channel_id,
        owner_id,
        "Channel `#{channel_name}` claimed by #{username}"
      )
      |> IO.inspect()
      ...
      {:ok} =
        Api.edit_channel_permissions(
          channel_id,
          userid,
          %{type: "member", allow: DiscordHr.Voice.new_voice_permissions()},
          "Channel `#{channel_name}` claimed by #{username}"
        )
