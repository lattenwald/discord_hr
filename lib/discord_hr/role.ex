defmodule DiscordHr.Role do
  ...
  def interaction_react(
    ...
    {text, placeholder, errtext} =
      case locale do
        "ru" <> _ ->
          {"Выберите роли из `#{name}`", "Выберите роли", "Пизда рулю, ошибка"}

        _ ->
          {"Set your `#{name}` roles", "Choose roles", "Stuff fucked up, something went wrong"}
      end
    ...
    with %{roles: group_roles, max: max, enabled: true} <- Storage.get([guild_id, @key, name]) do
      ...
      options =
        ...
        |> Enum.map(
          &%Component.Option{
            label: all_roles[&1].name,
            value: &1,
            default: Enum.member?(user_roles, &1)
          }
        )

      menu =
        Component.SelectMenu.select_menu("role:select:#{name}",
          placeholder: placeholder,
          options: options,
          min_values: 0,
          max_values: min(max, length(options))
        )
      ...
      buttons_row =
        Component.ActionRow.action_row(
          components: [Component.Button.interaction_button("Cancel", "cancel", style: 1)]
        )
  ...
  def component_react(
    ...
    |> Enum.each(
      &Api.add_guild_member_role(
        ...
        user_id,
        &1,
        "#{username} selected roles in group #{name}"
      )
    )
  ...
  def update_guild_application_command(guild_id) do
    with {:ok, commands} <- Api.get_guild_application_commands(guild_id),
         %{id: command_id} <- Enum.find(commands, fn %{name: name} -> name == "role" end) do
      ...
      case Api.edit_guild_application_command(guild_id, command_id, command) do
        {:error, %{status_code: 429, response: %{retry_after: retry_after}}} ->
          Logger.warning(
            "failed setting guild application command for guild #{guild_id}, will retry in #{retry_after} seconds"
          )

          :timer.apply_after(
            floor(retry_after * 1000) + 100,
            __MODULE__,
        ...
        {:error, %{response: %{message: message}}} ->
          ...
          Logger.warning(
            "failed setting guild application command for guild #{guild_id}: #{message} will retry in #{retry_after} seconds"
          )
